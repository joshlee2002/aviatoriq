...
const checks: EnvCheck[] = [
  ...
  },
  {
    ...
    hint: "Set OPENAI_API_KEY or BUILT_IN_FORGE_API_KEY",
  },
...
export function validateEnv(): void {
  ...
  for (const check of checks) {
    ...
    if (check.key === "OPENAI_API_KEY") {
      if (!hasLLMKey) {
        errors.push(
          `[ENV] MISSING: OPENAI_API_KEY (or BUILT_IN_FORGE_API_KEY) — ${check.description}`
        );
      }
      continue;
    }
    ...
    if (!value) {
      if (check.required) {
        errors.push(`[ENV] MISSING (required): ${check.key} — ${check.description}`);
        if (check.hint) errors.push(`       Hint: ${check.hint}`);
      } else {
        warnings.push(`[ENV] MISSING (optional): ${check.key} — ${check.description}`);
      }
      continue;
    }
