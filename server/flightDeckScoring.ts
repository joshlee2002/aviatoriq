...
// Top-of-funnel quiz. Fun surface, serious scoring underneath.
// No contact details required — result is immediate and shareable.

export type FlightDeckInput = {
  dream: string; // airline_captain | cargo | private_jet | fun | not_sure
  barrier: string; // cost | time | medical | confidence | no_idea
  age: string; // under18 | 18_25 | 26_35 | 36_45 | over45
  budget: string; // none | some | moderate | serious | funded
  experience: string; // none | trial | ppl | hours_50plus
  timeline: string; // asap | one_year | two_five | someday | not_sure
};

export type FlightDeckResult = {
  score: number; // 0–100 "Flight Potential" score
  phase: "Exploration" | "Development" | "Flight Ready";
  headline: string; // e.g. "You're closer than most people think."
  subheadline: string; // personalised 1-sentence summary
  biggestBarrier: string; // human-readable barrier label
  barrierAdvice: string; // 1-sentence actionable advice
  estimatedTimeline: string;
  recommendedRoute: string;
  shareText: string; // pre-written social share copy
};
...
function scoreDream(dream: string): number {
  // Professional goals score higher (more data value + more achievable path)
...
function scoreBarrier(barrier: string): number {
  // Lower barrier = higher score (cost is the most common, least disqualifying)
  if (barrier === "cost") return 18;
  if (barrier === "time") return 15;
  if (barrier === "no_idea") return 12;
  if (barrier === "confidence") return 10;
  if (barrier === "medical") return 8; // medical uncertainty is the hardest to resolve
  return 12;
}

function scoreAge(age: string): number {
  // Younger = more time, but all ages are viable
  if (age === "18_25") return 20;
  if (age === "26_35") return 18;
  if (age === "under18") return 16;
  if (age === "36_45") return 12;
  return 8; // over45 — still possible, just more constrained
}
...
function getBarrierAdvice(barrier: string): string {
  const map: Record<string, string> = {
    ...
    medical:
      "A Class 1 medical is required for airline flying, but most common health conditions don't automatically disqualify you — an AME assessment takes less than a day.",
    confidence:
      "Every airline pilot started with zero hours. Confidence comes from taking the first step — a trial lesson is less than £200 and changes most people's perspective entirely.",
    no_idea:
      "The most common starting point. A free career assessment takes 10 minutes and gives you a personalised roadmap, recommended route, and matched schools.",
  };
  return (
    map[barrier] ??
    "The best next step is a structured assessment to understand your specific situation."
  );
}

function getTimeline(score: number, timeline: string, age: string): string {
  if (timeline === "asap" && score >= 60)
    return "You could realistically begin training within the next 3–6 months.";
  if (score >= 75)
    return "Based on your profile, you could be qualified within 2–3 years.";
  if (score >= 55)
    return "With the right preparation, qualification is realistic within 3–5 years.";
  if (age === "over45")
    return "Training is still very achievable — a PPL or modular CPL route is well within reach.";
...
function getRoute(dream: string, budget: string, timeline: string): string {
  ...
  if (budget === "funded" || budget === "serious") {
    if (timeline === "asap" || timeline === "one_year")
      return "Integrated ATPL (fastest route to airline)";
    return "Modular ATPL (flexible, lower cost)";
  }
  if (dream === "airline_captain" || dream === "cargo")
    return "Modular ATPL (build qualifications in stages)";
  if (dream === "private_jet")
    return "CPL + IR (commercial licence with instrument rating)";
...
function getSubheadline(
  ...
  if (score >= 75)
    return `Your profile matches someone who could realistically become a ${label} — your biggest challenge is taking the next step.`;
  if (score >= 55)
    return `You have the right mindset for a career as a ${label}. Your main obstacle is ${getBarrierLabel(barrier).toLowerCase()}, which is solvable.`;
  if (score >= 40)
    return `Becoming a ${label} is achievable from your position — but it will require a clear plan and some preparation first.`;
  if (age === "under18")
    return `You have more time than almost anyone. Starting to build your knowledge now puts you ahead of 90% of aspiring pilots.`;
...
export function scoreFlightDeckQuiz(input: FlightDeckInput): FlightDeckResult {
  ...
  // Max possible: 20+18+20+22+10+10 = 100
