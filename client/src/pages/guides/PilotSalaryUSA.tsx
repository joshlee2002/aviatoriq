...
const PilotSalaryUSA = () => {
  return (
    <GuideLayout
      ...
      canonical="https://aviatorpath.com/us/guides/airline-pilot-salary-usa"
      ...
      sections={[
        {
          ...
          content: (
            <>
              ...
              <div className="p-4 rounded-xl my-4" style={{ background: "oklch(0.72 0.18 65 / 0.08)", border: "1px solid oklch(0.72 0.18 65 / 0.25)" }}>
                <p className="text-sm font-semibold mb-2" style={{ color: "oklch(0.85 0.15 65)" }}>How to read the salary figures in this guide</p>
                <div className="overflow-x-auto">
                  <table className="w-full text-xs mt-2">
                    <thead>
                      <tr style={{ borderBottom: "1px solid oklch(1 0 0 / 0.1)" }}>
                        <th className="text-left py-2 pr-4 font-semibold text-white">Pay Component</th>
                        <th className="text-left py-2 pr-4 font-semibold text-white">What It Means</th>
                        <th className="text-left py-2 font-semibold text-white">Caveat</th>
                      </tr>
                    </thead>
                    <tbody>
                      {[
                        ...
                        <tr key={i} style={{ borderBottom: "1px solid oklch(1 0 0 / 0.06)" }}>
                          <td className="py-2 pr-4 font-medium text-white">{row.component}</td>
                          <td className="py-2 pr-4" style={{ color: "oklch(0.7 0.04 240)" }}>{row.meaning}</td>
                          <td className="py-2" style={{ color: "oklch(0.6 0.04 240)" }}>{row.caveat}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <p className="text-sm p-3 rounded-lg my-3" style={{ background: "oklch(0.45 0.18 240 / 0.07)", border: "1px solid oklch(0.45 0.18 240 / 0.15)" }}>
                Want a scenario-based salary and career projection? Generate your free <Link href="/us/roadmap" className="underline" style={{ color: "oklch(0.72 0.18 240)" }}>personalised US pilot career roadmap</Link> to see a timeline from training to captain.
              </p>
        ...
        },
        {
          ...
          content: (
            <>
              <p>
                The following table shows the verified sources used for salary data in this guide, including the source URL, last checked date, pay basis, and caveat for each data point. All figures are indicative ranges — verify current pay scales directly with airlines or via ALPA before making any financial decision.
              </p>
              <SalarySourceTable country="United States" className="my-6" />
              <div className="p-4 rounded-xl mt-4" style={{ background: "oklch(0.65 0.18 30 / 0.1)", border: "1px solid oklch(0.65 0.18 30 / 0.25)" }}>
                <p className="text-sm font-semibold mb-1" style={{ color: "oklch(0.8 0.15 50)" }}>Seniority caveat — read before using any figure</p>
                <p className="text-sm" style={{ color: "oklch(0.7 0.04 240)" }}>
                  All pay ranges in this guide are scenario illustrations based on publicly reported CBA data. Actual pay depends on: airline, seat (FO/CA), fleet, base, credit hours, guarantee, seniority number, contract year, and the state of the hiring cycle. A downturn, furlough, or hiring pause can change upgrade timelines and pay expectations significantly. <strong className="text-white">Do not use any figure in this guide as a guaranteed earnings projection.</strong>
                </p>
              </div>
            </>
          ),
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr style={{ background: "oklch(0.16 0.09 250)", borderBottom: "1px solid oklch(1 0 0 / 0.1)" }}>
                      <th className="text-left px-4 py-3 font-semibold text-white">Pay Component</th>
                      <th className="text-left px-4 py-3 font-semibold text-white">Indicative Range (2026)</th>
                  ...
                  <tbody>
                    {[
                      { component: "Regional FO starting pay", range: "$85,000–$100,000+/yr", basis: "Annual (base + per diem)", caveat: "Figures include base pay and per diem. Actual pay depends on airline, domicile, and contract. Verify via ALPA or airline careers pages." },
                      { component: "Regional FO signing bonus", range: "$10,000–$75,000 (varies widely)", basis: "One-time", caveat: "Bonuses change with staffing needs. May have repayment obligation if you leave within 1–2 years. Verify terms before accepting." },
                      { component: "Regional captain upgrade timeline", range: "1–5 years (varies by carrier)", basis: "Scenario", caveat: "Treat any upgrade timeline claim as a scenario, not a promise. Depends on fleet growth, retirements, and hiring pauses." },
                      { component: "Regional captain pay", range: "$120,000–$180,000/yr", basis: "Annual (base)", caveat: "Indicative range. Verify current CBA rates via ALPA or airline careers pages." },
                    ].map((row, i) => (
                      <tr key={i} style={{ borderBottom: "1px solid oklch(1 0 0 / 0.06)", background: i % 2 === 0 ? "transparent" : "oklch(1 0 0 / 0.02)" }}>
                        <td className="px-4 py-3 font-medium text-white align-top">{row.component}</td>
                        <td className="px-4 py-3 font-semibold align-top" style={{ color: "oklch(0.85 0.15 65)" }}>{row.range}</td>
                        <td className="px-4 py-3 align-top text-xs" style={{ color: "oklch(0.65 0.04 240)" }}>{row.basis}</td>
                        <td className="px-4 py-3 align-top text-xs" style={{ color: "oklch(0.6 0.04 240)" }}>{row.caveat}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        },
        {
          ...
          content: (
            <>
              <p>
                The timeline from zero hours to major airline captain has compressed for some pilots in recent hiring cycles. A typical career arc in 2026 looks like this — but treat all timelines as scenarios, not promises:
              </p>
              <div className="overflow-x-auto my-6 rounded-xl" style={{ border: "1px solid oklch(1 0 0 / 0.08)" }}>
                <table className="w-full text-sm">
                  <thead>
                    <tr style={{ background: "oklch(0.16 0.09 250)", borderBottom: "1px solid oklch(1 0 0 / 0.1)" }}>
                      <th className="text-left px-4 py-3 font-semibold text-white">Career Stage</th>
                      <th className="text-left px-4 py-3 font-semibold text-white">Approximate Timeline</th>
                      <th className="text-left px-4 py-3 font-semibold text-white">Indicative Pay Range</th>
                      <th className="text-left px-4 py-3 font-semibold text-white">Key Variables</th>
                    </tr>
                  </thead>
                  <tbody>
                    {[
                      ...
                      <tr key={i} style={{ borderBottom: "1px solid oklch(1 0 0 / 0.06)", background: i % 2 === 0 ? "transparent" : "oklch(1 0 0 / 0.02)" }}>
                        ...
                        <td className="px-4 py-3 align-top" style={{ color: "oklch(0.7 0.04 240)" }}>{row.timeline}</td>
                        <td className="px-4 py-3 font-semibold align-top" style={{ color: "oklch(0.85 0.15 65)" }}>{row.pay}</td>
                        <td className="px-4 py-3 align-top text-xs" style={{ color: "oklch(0.6 0.04 240)" }}>{row.variables}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <p>
                These timelines are approximate and depend heavily on the state of the industry, the specific airline, fleet, base and individual seniority. A downturn or hiring pause can quickly change upgrade expectations.
              </p>
              <p>
        ...
        },
        {
          ...
          content: (
            <>
              <p>
                Pilot compensation is more complex than base salary alone. Understanding the full picture helps you evaluate offers and plan your career. Every component below has a caveat — none are guaranteed, and all vary by airline, contract, and year.
              </p>
              <div className="overflow-x-auto my-6 rounded-xl" style={{ border: "1px solid oklch(1 0 0 / 0.08)" }}>
                <table className="w-full text-sm">
                  <thead>
                    <tr style={{ background: "oklch(0.16 0.09 250)", borderBottom: "1px solid oklch(1 0 0 / 0.1)" }}>
                      ...
                      <th className="text-left px-4 py-3 font-semibold text-white">Indicative Value</th>
                      <th className="text-left px-4 py-3 font-semibold text-white">Caveat</th>
                    </tr>
                  </thead>
                  <tbody>
                    {[
                      ...
                      <tr key={i} style={{ borderBottom: "1px solid oklch(1 0 0 / 0.06)", background: i % 2 === 0 ? "transparent" : "oklch(1 0 0 / 0.02)" }}>
                        <td className="px-4 py-3 font-medium text-white align-top">{row.component}</td>
                        <td className="px-4 py-3 font-semibold align-top" style={{ color: "oklch(0.85 0.15 65)" }}>{row.value}</td>
                        <td className="px-4 py-3 align-top text-xs" style={{ color: "oklch(0.6 0.04 240)" }}>{row.caveat}</td>
                      </tr>
                    ))}
                  </tbody>
