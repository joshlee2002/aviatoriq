...
import { GraduationCap, ArrowRight, AlertTriangle, CheckCircle2, XCircle } from "lucide-react";
import { CADET_PROGRAMMES_2026 } from "@/data/cadetProgrammes2026";

const CADET_GUIDES = [
  { href: "/guides/ba-speedbird-academy", label: "BA Speedbird Academy", flag: "🇬🇧", status: "Open 2026", funded: true },
  { href: "/guides/easyjet-generation", label: "easyJet Generation easyJet", flag: "🇬🇧", status: "Open", funded: false },
  { href: "/guides/ryanair-cadet", label: "Ryanair Cadet Pilot", flag: "🇮🇪", status: "Open", funded: false },
  { href: "/guides/wizz-air-academy", label: "Wizz Air Academy", flag: "🇭🇺", status: "Open", funded: false },
  { href: "/guides/tui-mpl", label: "TUI MPL Programme", flag: "🇬🇧", status: "Paused 2026", funded: true },
  { href: "/guides/emirates-cadet-pilot-program", label: "Emirates Cadet Programme", flag: "🇦🇪", status: "Open", funded: true },
  { href: "/guides/qantas-group-pilot-academy", label: "Qantas Group Pilot Academy", flag: "🇦🇺", status: "Open", funded: false },
  { href: "/guides/air-canada-jazz-cadet", label: "Air Canada Jazz Approach", flag: "🇨🇦", status: "Open", funded: false },
];
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6">
          ...
          <div className="text-center mb-12">
            ...
            <p className="text-white/70 max-w-2xl mx-auto text-lg">
              A complete guide to airline-sponsored and airline-linked pilot training programmes. Understand the difference between funded and self-funded pathways before you apply.
            </p>
          </div>
          ...
          <div className="bg-amber-900/30 border border-amber-500/30 rounded-xl p-5 mb-10 flex gap-3">
            ...
            <div>
              <p className="font-semibold text-amber-300 mb-1">Funded vs Self-Funded: The Most Important Distinction</p>
              <p className="text-sm text-amber-200">
                Not all "cadet programmes" are funded. Some airlines offer a conditional job offer but require you to self-fund training through a loan. Always confirm whether the airline is paying for your training or whether you are taking out a loan with a conditional offer attached. These are fundamentally different financial propositions.
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-12">
            <div className="bg-green-900/20 rounded-xl border border-green-500/20 p-5">
              <div className="flex items-center gap-2 mb-3">
                ...
                <p className="font-semibold text-green-400">Genuinely Funded Programmes</p>
              </div>
              <ul className="space-y-2 text-sm text-white/70">
                ...
                <li>Extremely competitive selection (often &lt;1% acceptance)</li>
            ...
            <div className="bg-orange-900/20 rounded-xl border border-orange-500/20 p-5">
              <div className="flex items-center gap-2 mb-3">
                ...
                <p className="font-semibold text-orange-400">Self-Funded with Conditional Offer</p>
              </div>
              <ul className="space-y-2 text-sm text-white/70">
                <li>You take out a loan (typically £80k–£120k)</li>
                <li>Airline offers a job if you pass all stages</li>
                <li>Financial risk is yours if you fail or the offer changes</li>
          ...
          <h2 className="font-display font-bold text-2xl mb-4">Programme Status (2026)</h2>
          <div className="bg-white/5 rounded-xl border border-white/10 overflow-hidden mb-10">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-white/10">
                    <th className="text-left px-4 py-3 text-white/60 font-medium">Programme</th>
                    <th className="text-left px-4 py-3 text-white/60 font-medium">Status</th>
                    <th className="text-left px-4 py-3 text-white/60 font-medium">Funded?</th>
                    <th className="text-left px-4 py-3 text-white/60 font-medium">Guide</th>
                  </tr>
                </thead>
                <tbody>
                  {CADET_GUIDES.map((prog, i) => (
                    <tr key={prog.href} className={`border-b border-white/5 ${i % 2 === 0 ? "" : "bg-white/[0.02]"}`}>
                      <td className="px-4 py-3 text-white">
                        <span className="mr-2">{prog.flag}</span>{prog.label}
                      </td>
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${
                          prog.status.includes("Open") ? "bg-green-900/40 text-green-300" : "bg-amber-900/40 text-amber-300"
                        }`}>
                          {prog.status}
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <Link href={prog.href} className="text-blue-400 hover:text-blue-300 transition-colors inline-flex items-center gap-1 text-xs">
            ...
            <p className="text-xs text-white/30 px-4 py-3 border-t border-white/5">
              Status is based on publicly available information as of 2026. Always verify current status directly with the airline before applying.
            </p>
          </div>

          {/* Key Questions */}
          <h2 className="font-display font-bold text-2xl mb-4">Key Questions Before You Apply</h2>
          ...
          <div className="bg-gradient-to-r from-blue-900/40 to-blue-800/20 rounded-xl border border-blue-500/20 p-6 text-center">
            <h3 className="font-display font-bold text-xl mb-2">Check Your Eligibility</h3>
            <p className="text-white/60 text-sm mb-4">Use our cadet eligibility checker to see which programmes you may qualify for based on your age, qualifications, and experience.</p>
            <Link href="/tools/cadet-eligibility" className="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-lg text-sm font-medium transition-colors">
