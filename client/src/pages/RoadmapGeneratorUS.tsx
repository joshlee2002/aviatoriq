...
const pathGradients: Record<
  PathIdUS,
  { from: string; to: string; glow: string; badge: string; badgeText: string }
> = {
  cadet: {
...
const pathIcons: Record<PathIdUS, string> = {
  cadet: "🏆",
  part141: "⚡",
  part61: "🧩",
  university: "🎓",
...
export default function RoadmapGeneratorUS() {
  const [currentStep, setCurrentStep] = useState(-1);
  const [answers, setAnswers] = useState<Record<string, string>>({});
  const [result, setResult] = useState<PathIdUS | null>(null);
  ...
  const handleAnswer = (questionId: string, value: string) => {
    ...
    if (currentStep < roadmapQuestionsUS.length - 1) {
  ...
  const calculateResult = (finalAnswers: Record<string, string>) => {
    ...
    setCurrentStep(roadmapQuestionsUS.length);
    setCalcPhase(0);

    const scores: Record<PathIdUS, number> = {
      cadet: 0,
      part141: 0,
      part61: 0,
      university: 0,
    ...
    roadmapQuestionsUS.forEach(q => {
      const option = q.options.find(o => o.value === finalAnswers[q.id]);
      if (option?.points) {
        Object.entries(option.points).forEach(([path, pts]) => {
          scores[path as PathIdUS] += pts as number;
        });
      }
    });

    let winningPath: PathIdUS = "part141";
    ...
    if (age === "under18") {
      winningPath = "future";
    } else if (goal === "recreational") {
      winningPath = "part61";
    } else if (time === "parttime") {
      winningPath = "part61";
    } else if (budget === "top" && age !== "over46") {
      winningPath = "university";
    } else if (budget === "low" && age !== "over46") {
      winningPath = "cadet";
    } else if (
      (budget === "veryhigh" || budget === "high") &&
      time === "fulltime" &&
      (goal === "major" || goal === "regional")
    ) {
      winningPath = "part141";
    } else if (budget === "medium" && (goal === "major" || goal === "cadet")) {
      winningPath = "cadet";
    } else {
      let maxScore = -1;
    ...
    >
      ...
      <main className="flex-1">
        <AnimatePresence mode="wait">
            ...
            >
              ...
              >
                ...
                <div
                  ...
                  style={{
                    background:
                      "radial-gradient(circle, oklch(0.45 0.18 240 / 0.12) 0%, transparent 70%)",
                  }}
                />
                ...
                <div className="container max-w-4xl text-center relative py-20 px-4">
                  ...
                  >
                    ...
                    >
                      ...
                      <span
                        style={{
                          background:
                            "linear-gradient(135deg, oklch(0.65 0.18 240), oklch(0.72 0.18 200))",
                    ...
                    <p className="text-xl text-white/60 mb-12 max-w-2xl mx-auto leading-relaxed">
                      Part 141 or Part 61? Airline cadet or university R-ATP?
                      Answer 5 honest questions about your age, budget, and
                      goals — and we'll generate a highly specific, data-driven
                      US training roadmap built for your exact situation.
                    </p>
                  </motion.div>
                  ...
                  >
                    {[
                      ...
                      },
                      {
                        ...
                        label: "Matched US schools",
                      },
                      {
                        icon: <Target className="w-4 h-4" />,
                        label: "5-step action plan",
                      },
                      {
                  ...
                  >
                    <button
                      ...
                      style={{
                        background:
                          "linear-gradient(135deg, oklch(0.50 0.22 255), oklch(0.45 0.20 240))",
                        boxShadow:
                          "0 0 40px oklch(0.50 0.22 255 / 0.4), 0 8px 32px oklch(0 0 0 / 0.3)",
                      }}
                    >
                  ...
                  >
                    {[
                      { value: "5 Paths", label: "FAA-specific routes" },
                      { value: "2026", label: "Updated US costs" },
          ...
          {/* ─── QUESTION FLOW ─── */}
          {currentStep >= 0 && currentStep < roadmapQuestionsUS.length && (
            <motion.div
              key={`q-${currentStep}`}
              ...
              style={{
                background:
                  "linear-gradient(160deg, oklch(0.10 0.10 255) 0%, oklch(0.12 0.08 252) 100%)",
              }}
            >
              <div className="w-full max-w-2xl">
                <div className="mb-10">
                  <div className="flex justify-between items-center mb-3">
                    ...
                    >
                      Question {currentStep + 1} of {roadmapQuestionsUS.length}
                    </span>
                    <span
                    ...
                    >
                      {Math.round(
                        ((currentStep + 1) / roadmapQuestionsUS.length) * 100
                      )}
                      %
                    </span>
                  </div>
                  ...
                  >
                    <motion.div
                      ...
                      style={{
                        background:
                          "linear-gradient(90deg, oklch(0.50 0.22 255), oklch(0.60 0.22 200))",
                      }}
                      initial={{
                        width: `${(currentStep / roadmapQuestionsUS.length) * 100}%`,
                      }}
                      animate={{
                        width: `${((currentStep + 1) / roadmapQuestionsUS.length) * 100}%`,
                      }}
                      transition={{ duration: 0.4 }}
                    />
                  </div>
                  <div className="flex justify-between mt-3">
                    {roadmapQuestionsUS.map((_, i) => (
                      <div
                        key={i}
                ...
                >
                  {roadmapQuestionsUS[currentStep].title}
                </h2>

                <div className="space-y-3">
                  {roadmapQuestionsUS[currentStep].options.map(
                    (option, idx) => (
                      <motion.button
                        key={idx}
                        ...
                        onClick={() =>
                          handleAnswer(
                            roadmapQuestionsUS[currentStep].id,
                            option.value
                          )
                        }
                        ...
                        onMouseEnter={e => {
                          ...
                          (e.currentTarget as HTMLElement).style.border =
                            "1px solid oklch(0.50 0.22 255 / 0.35)";
              ...
              <div className="text-center max-w-md">
                <div className="relative w-20 h-20 mx-auto mb-8">
                  <div
                    ...
                    style={{
                      borderColor: "oklch(0.50 0.22 255 / 0.15)",
                      borderTopColor: "oklch(0.65 0.18 240)",
                    }}
                  />
                  <div
                    className="absolute inset-3 rounded-full flex items-center justify-center"
                    style={{ background: "oklch(0.50 0.22 255 / 0.1)" }}
                  >
                    <Plane
                      className="w-6 h-6"
                      style={{ color: "oklch(0.65 0.18 240)" }}
                ...
                <div className="space-y-2 mt-6">
                  {[
                    "Cross-referencing 2026 FAA training costs…",
                    "Matching airline cadet requirements…",
                    "Building your personalised US roadmap…",
                  ].map((msg, i) => (
                    <motion.div
                      key={i}
                      initial={{ opacity: 0, x: -10 }}
                      animate={{ opacity: calcPhase > i ? 1 : 0.2, x: 0 }}
                        ...
                        style={{
                          background:
                            calcPhase > i
                              ...
                              : "oklch(1 0 0 / 0.05)",
                          border: `1px solid ${calcPhase > i ? "oklch(0.55 0.22 145 / 0.4)" : "oklch(1 0 0 / 0.1)"}`,
                        }}
                      >
                        {calcPhase > i && (
                          <CheckCircle2
          ...
          {result && !isCalculating && grad && (
            ...
            >
              ...
              <div
                ...
                style={{
                  background:
                    "linear-gradient(160deg, oklch(0.10 0.10 255) 0%, oklch(0.13 0.10 252) 100%)",
                }}
              >
                <div
                  className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[700px] h-[700px] rounded-full pointer-events-none"
                  style={{
                    background: `radial-gradient(circle, ${grad.glow} 0%, transparent 65%)`,
                  }}
                />
                <div
                  className="absolute inset-0 pointer-events-none"
                ...
                <div className="container max-w-3xl text-center relative">
                  ...
                  >
                    ...
                    <div
                      className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-bold uppercase tracking-widest mb-5"
                      style={{
                        background: grad.badge,
                        border: `1px solid ${grad.badgeText}40`,
                        color: grad.badgeText,
                      }}
                    >
                      {roadmapResultsUS[result].tagline}
                    </div>
                    <h1
                      className="text-4xl md:text-5xl lg:text-6xl font-display font-bold text-white mb-6"
                      style={{ letterSpacing: "-0.03em" }}
                    >
                      {roadmapResultsUS[result].title}
                    </h1>
                    <p
                      className="text-lg md:text-xl leading-relaxed max-w-2xl mx-auto"
                      style={{ color: "oklch(0.70 0.04 240)" }}
                    >
                      {roadmapResultsUS[result].description}
                    </p>
                  </motion.div>
                    ...
                    {[
                      {
                        ...
                        value: roadmapResultsUS[result].cost,
                      },
                      {
                        icon: <Clock className="w-5 h-5" />,
                        label: "Timeline",
                        value: roadmapResultsUS[result].timeline,
                      },
                      {
                        icon: <Shield className="w-5 h-5" />,
                        label: "Financial Risk",
                        value: roadmapResultsUS[result].risk,
                      },
                    ].map((stat, i) => (
                      <div
                        key={i}
                ...
                <div className="container max-w-3xl px-4 py-16 space-y-12">
                  ...
                  >
                    <div className="flex items-center gap-3 mb-6">
                      <div
                        ...
                        style={{
                          background: grad.badge,
                          border: `1px solid ${grad.badgeText}30`,
                        }}
                      >
                        <Target
                          className="w-4 h-4"
                    ...
                    <div className="space-y-3">
                      {roadmapResultsUS[result].steps.map((step, idx) => (
                        <motion.div
                          key={idx}
                          initial={{ opacity: 0, x: -10 }}
                          animate={{ opacity: 1, x: 0 }}
                        ...
                        >
                          ...
                          <p className="text-white/75 leading-relaxed pt-1 flex-1">
                            {step}
                          </p>
                        </motion.div>
                      ))}
                  ...
                  >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      {roadmapResultsUS[result].schools.map((school, idx) => (
                        <motion.div
                          key={idx}
                          initial={{ opacity: 0, y: 10 }}
                          animate={{ opacity: 1, y: 0 }}
                  ...
                  >
                    <div
                      ...
                      style={{
                        ...
                        border: "1px solid oklch(0.65 0.20 50 / 0.25)",
                      }}
                    >
                      ...
                      >
                        {roadmapResultsUS[result].watchOut}
                      </p>
                    </div>
                  </motion.div>
                  ...
                  >
                    <div
                      ...
                      style={{
                        background:
                          "linear-gradient(135deg, oklch(0.14 0.12 255), oklch(0.17 0.10 248))",
                    ...
                    >
                      ...
                      <h3 className="text-2xl font-display font-bold text-white mb-3">
                        Want your full personalised assessment?
                      </h3>
                      <p
                        className="text-sm mb-2 max-w-md mx-auto"
                        style={{ color: "oklch(0.60 0.04 240)" }}
                      >
                        This roadmap is a starting point. The Free Assessment
                        goes deeper — it analyses your exact barriers, matches
                        you with specific US schools, and generates a
                      ...
                      <p
                        ...
                        style={{ color: "oklch(0.65 0.18 240)" }}
                      >
                        Free · Takes 5 minutes · No obligation
                      </p>
                      <Link
                        href="/quiz"
                        ...
                        style={{
                          background:
                            "linear-gradient(135deg, oklch(0.72 0.18 65), oklch(0.62 0.2 45))",
                          boxShadow: "0 4px 24px oklch(0.72 0.18 65 / 0.35)",
                        }}
                      >
                      ...
                      <div className="flex flex-wrap justify-center gap-3 mt-6">
                        <Link
                          href="/us/schools"
                          className="inline-flex items-center gap-1 text-xs no-underline transition-all"
                          style={{ color: "oklch(0.50 0.04 240)" }}
                        >
                          Browse US Schools <ArrowRight className="w-3 h-3" />
                        </Link>
                        ...
                        <Link
                          ...
                          style={{ color: "oklch(0.50 0.04 240)" }}
                        >
                          US Cost Calculator <ArrowRight className="w-3 h-3" />
                        </Link>
                        <span style={{ color: "oklch(0.30 0 0)" }}>·</span>
                        <Link
                          href="/us/guides"
                          ...
                          style={{ color: "oklch(0.50 0.04 240)" }}
                        >
                          US Training Guides <ArrowRight className="w-3 h-3" />
                        </Link>
                      </div>
                    </div>
                  </motion.div>
