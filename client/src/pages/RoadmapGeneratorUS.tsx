...
import {
  Share2, CheckCircle2, ArrowRight, AlertTriangle, MapPin, Clock,
...
const pathGradients: Record<PathIdUS, { from: string; to: string; glow: string; badge: string; badgeText: string }> = {
  cadet: {
...
export default function RoadmapGeneratorUS() {
  ...
  const calculateResult = (finalAnswers: Record<string, string>) => {
    ...
    const scores: Record<PathIdUS, number> = { cadet: 0, part141: 0, part61: 0, university: 0, future: 0 };
    ...
    if (age === "under18") {
      ...
      winningPath = "cadet";
    } else if ((budget === "veryhigh" || budget === "high") && time === "fulltime" && (goal === "major" || goal === "regional")) {
      winningPath = "part141";
      ...
      Object.entries(scores).forEach(([path, score]) => {
        if (score > maxScore) { maxScore = score; winningPath = path as PathIdUS; }
      });
    }
  ...
  const handleShare = async () => {
    ...
    if (navigator.share) {
      try { await navigator.share({ title: "My US Pilot Training Roadmap — AviatorIQ", text, url }); } catch {}
    } else {
                ...
                <div className="container max-w-4xl text-center relative py-20 px-4">
                  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
                    ...
                    <p className="text-xl text-white/60 mb-12 max-w-2xl mx-auto leading-relaxed">
                      Part 141 or Part 61? Airline cadet or university R-ATP? Answer 5 honest questions about your age, budget, and goals — and we'll generate a highly specific, data-driven US training roadmap built for your exact situation.
                    </p>
                  </motion.div>
                  ...
                  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }} className="flex flex-wrap justify-center gap-3 mb-12">
                    {[
                      { icon: <DollarSign className="w-4 h-4" />, label: "Real 2026 US costs" },
                      { icon: <Clock className="w-4 h-4" />, label: "Realistic timelines" },
                      { icon: <MapPin className="w-4 h-4" />, label: "Matched US schools" },
                      ...
                      <div key={i} className="flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium"
                        style={{ background: "oklch(1 0 0 / 0.05)", border: "1px solid oklch(1 0 0 / 0.08)", color: "oklch(0.75 0.04 240)" }}>
            ...
            >
              <div className="w-full max-w-2xl">
                <div className="mb-10">
                  <div className="flex justify-between items-center mb-3">
                    ...
                    <span className="text-sm font-bold" style={{ color: "oklch(0.65 0.18 240)" }}>
                      {Math.round(((currentStep + 1) / roadmapQuestionsUS.length) * 100)}%
                    </span>
                  </div>
                  <div className="h-1.5 rounded-full overflow-hidden" style={{ background: "oklch(1 0 0 / 0.06)" }}>
                    <motion.div
                      ...
                      initial={{ width: `${(currentStep / roadmapQuestionsUS.length) * 100}%` }}
                      animate={{ width: `${((currentStep + 1) / roadmapQuestionsUS.length) * 100}%` }}
                ...
                <div className="space-y-3">
                  {roadmapQuestionsUS[currentStep].options.map((option, idx) => (
                    <motion.button
                      key={idx}
                      initial={{ opacity: 0, y: 10 }}
                      animate={{ opacity: 1, y: 0 }}
                      transition={{ delay: idx * 0.06 }}
                      onClick={() => handleAnswer(roadmapQuestionsUS[currentStep].id, option.value)}
                    ...
                    >
                      ...
                      <ChevronRight className="w-5 h-5 flex-shrink-0" style={{ color: "oklch(0.40 0.04 240)" }} />
                    </motion.button>
                  ))}
                </div>
              </div>
              ...
              <div className="relative overflow-hidden py-20 px-4" style={{ background: "linear-gradient(160deg, oklch(0.10 0.10 255) 0%, oklch(0.13 0.10 252) 100%)" }}>
                ...
                <div className="container max-w-3xl text-center relative">
                  ...
                  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}
                    ...
                    {[
                      { icon: <DollarSign className="w-5 h-5" />, label: "Estimated Cost", value: roadmapResultsUS[result].cost },
                      { icon: <Clock className="w-5 h-5" />, label: "Timeline", value: roadmapResultsUS[result].timeline },
                      { icon: <Shield className="w-5 h-5" />, label: "Financial Risk", value: roadmapResultsUS[result].risk },
                    ].map((stat, i) => (
                ...
                <div className="container max-w-3xl px-4 py-16 space-y-12">
                  ...
                  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.65 }}>
                    <div className="p-6 rounded-2xl" style={{ background: "oklch(0.65 0.20 50 / 0.08)", border: "1px solid oklch(0.65 0.20 50 / 0.25)" }}>
                      ...
                      <p className="leading-relaxed" style={{ color: "oklch(0.70 0.08 55)" }}>{roadmapResultsUS[result].watchOut}</p>
                    </div>
                  </motion.div>
                  ...
                  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.75 }}>
                    <div className="p-8 rounded-2xl text-center" style={{ background: "linear-gradient(135deg, oklch(0.14 0.12 255), oklch(0.17 0.10 248))", border: "1px solid oklch(0.55 0.18 240 / 0.25)" }}>
                      ...
                      <p className="text-sm mb-2 max-w-md mx-auto" style={{ color: "oklch(0.60 0.04 240)" }}>
                        This roadmap is a starting point. The Free Assessment goes deeper — it analyses your exact barriers, matches you with specific US schools, and generates a personalised action plan.
                      </p>
                      ...
                      <div className="flex flex-wrap justify-center gap-3 mt-6">
                        <Link href="/us/schools" className="inline-flex items-center gap-1 text-xs no-underline transition-all" style={{ color: "oklch(0.50 0.04 240)" }}>
                          Browse US Schools <ArrowRight className="w-3 h-3" />
                        </Link>
                        <span style={{ color: "oklch(0.30 0 0)" }}>·</span>
                        <Link href="/us/calculator" className="inline-flex items-center gap-1 text-xs no-underline transition-all" style={{ color: "oklch(0.50 0.04 240)" }}>
                          US Cost Calculator <ArrowRight className="w-3 h-3" />
                        </Link>
                        <span style={{ color: "oklch(0.30 0 0)" }}>·</span>
                        <Link href="/us/guides" className="inline-flex items-center gap-1 text-xs no-underline transition-all" style={{ color: "oklch(0.50 0.04 240)" }}>
