...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border ${colors[status] ?? "bg-gray-50 text-gray-600 border-gray-100"}`}
    >
      {status}
    </span>
  );
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/50 overflow-y-auto py-8 px-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl my-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-[var(--color-border)]">
          <div>
            <h2 className="font-display font-bold text-xl text-[var(--color-navy)]">
              {lead.fullName}
            </h2>
            <div className="flex items-center gap-2 mt-1">
              ...
              {(lead.intentScore ?? 0) > 0 && (
                <span
                  ...
                  title="Intent Score — commercial readiness signal (admin only)"
                >
        ...
        <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
          ...
          {lead.writtenAnswer && (
            <div>
              ...
              <p className="text-sm text-[var(--color-muted-foreground)] leading-relaxed p-3 bg-[var(--color-muted)] rounded-lg">
                {lead.writtenAnswer}
              </p>
            </div>
          )}
          ...
          <div>
            ...
            {detailQuery.data?.notes.map(note => (
              <div
                key={note.id}
                className="p-3 rounded-lg bg-[var(--color-muted)] mb-2"
              >
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/50 overflow-y-auto py-8 px-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl my-auto">
        ...
        <div className="p-6 max-h-[70vh] overflow-y-auto space-y-6">
          {/* Add school form */}
          <div>
            <h3 className="font-display font-semibold text-[var(--color-navy)] mb-4">
              Add new school
            </h3>
              ...
              {[
                {
                  key: "name",
                  label: "School name *",
                  placeholder: "e.g. Oxford Aviation Academy",
                },
                {
                  key: "country",
                  label: "Country",
                ...
                <div key={f.key}>
                  <label className="block text-xs font-semibold text-[var(--color-muted-foreground)] mb-1">
                    {f.label}
                  </label>
                  <input
                    type="text"
              ...
              {[
                ...
                >
                  <input
                    type="checkbox"
                    checked={
                      (form as Record<string, unknown>)[f.key] as boolean
                    }
                    onChange={e =>
              ...
              <div className="flex items-center gap-2">
                ...
                <select
                  ...
                  onChange={e =>
                    setForm(p => ({
                      ...p,
                      financeAvailable: e.target.value as
                        | "yes"
                        | "no"
                        | "unknown",
                    }))
          ...
          <div>
            <h3 className="font-display font-semibold text-[var(--color-navy)] mb-3">
              Existing schools ({schoolsQuery.data?.length ?? 0})
            </h3>
            <div className="space-y-2">
              {schoolsQuery.data?.map(school => (
                ...
                >
                  <div>
                    ...
                    <div className="text-xs text-[var(--color-muted-foreground)]">
                      {school.country} · {school.active ? "Active" : "Inactive"}
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <button
      ...
      {waitlistQuery.isLoading ? (
        ...
        <div className="text-center py-12">
          ...
          <p className="text-sm text-[var(--color-muted-foreground)]">
            Applications submitted via /partner will appear here.
          </p>
        </div>
      ) : (
        <div className="divide-y divide-[var(--color-border)]">
          {entries.map(entry => (
            <div
              key={entry.id}
              className="p-5 hover:bg-[var(--color-muted)]/30 transition-colors"
            >
              <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="font-display font-bold text-[var(--color-navy)] text-sm">
                      {entry.schoolName}
                    </span>
                    {entry.country && (
                  ...
                  {entry.coursesOffered && (
                    <p className="text-xs text-[var(--color-muted-foreground)] mb-1">
                      <strong>Courses:</strong> {entry.coursesOffered}
                    </p>
                  )}
                  {entry.notes && (
                    <p className="text-xs text-[var(--color-muted-foreground)] bg-[var(--color-muted)] rounded-lg p-2 mt-1">
                      {entry.notes}
                    </p>
                  )}
                </div>
    ...
    <div className="card-base overflow-hidden">
      ...
      {introsQuery.isLoading ? (
        ...
        <div className="text-center py-12">
          ...
          <p className="text-sm text-[var(--color-muted-foreground)]">
            When leads request introductions from the results page, they will
            appear here.
          </p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[var(--color-border)] bg-[var(--color-muted)]">
                {["Lead ID", "School", "Status", "Requested", "Action"].map(
                  h => (
                    <th
                      key={h}
                      className="text-left px-4 py-3 text-xs font-semibold text-[var(--color-muted-foreground)] uppercase tracking-wider whitespace-nowrap"
                    >
                      {h}
                    </th>
                  )
              ...
              {intros.map(intro => (
                ...
                >
                  <td className="px-4 py-3">
                    <a
                      href={`/admin?lead=${intro.leadId}`}
                      className="text-[var(--color-primary)] font-medium hover:underline flex items-center gap-1"
                    >
                      Lead #{intro.leadId} <ArrowRight className="w-3 h-3" />
                    </a>
                  </td>
                  <td className="px-4 py-3 text-[var(--color-navy)] font-medium">
                    {intro.schoolName}
                  </td>
                  <td className="px-4 py-3">
                    <span
                      className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border ${statusColors[intro.status] ?? "bg-gray-50 text-gray-600 border-gray-100"}`}
                    >
                      {intro.status}
                    </span>
                  </td>
...
export default function AdminDashboard() {
  ...
  const launchStatsQuery = trpc.analytics.launchStats.useQuery(undefined, {
    enabled: isAuthenticated && user?.role === "admin",
    refetchInterval: 60_000,
  });
  const [page, setPage] = useState(1);
  const [sortBy, setSortBy] = useState<
    "createdAt" | "leadScore" | "intentScore"
  >("createdAt");
  ...
  const leadsQuery = trpc.admin.listLeads.useQuery(filters, {
    enabled: isAuthenticated && user?.role === "admin",
  });
  const exportQuery = trpc.admin.exportLeads.useQuery(undefined, {
    enabled: false,
  });
            ...
            return (
              <div className="mb-6 p-5 rounded-2xl border-2 border-[var(--color-primary)]/20 bg-gradient-to-br from-[var(--color-primary-light)] to-white">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <h2 className="font-display font-bold text-[var(--color-navy)] text-lg">
                      Launch Dashboard
                    </h2>
                  ...
                  <div className="flex items-center gap-2 text-xs text-[var(--color-muted-foreground)]">
                    <span>
                      All-time: {s.totalAllTime} assessments · {s.hotAllTime}{" "}
                      Flight Ready · {s.introAllTime} intro requests
                    </span>
                  </div>
                </div>
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
                  {[
                    ...
                    },
                    {
                      ...
                      value: `${s.introRate7d}%`,
                      sub: `${s.introLeads7d} leads`,
                      highlight: s.introRate7d >= 20,
                    },
                    {
                      label: "Avg Score",
                    ...
                    {
                      label: "Top Source",
                      value: s.topSource7d === "N/A" ? "—" : s.topSource7d,
                      sub:
                        s.topCountry7d === "N/A"
                          ? "top country"
                          : s.topCountry7d,
                    ...
                    >
                      <div
                        className={`text-2xl font-display font-bold ${m.highlight ? "text-white" : "text-[var(--color-navy)]"}`}
                      >
                        {m.value}
                      </div>
                      <div
                        className={`text-xs font-semibold mt-0.5 ${m.highlight ? "text-white/80" : "text-[var(--color-primary)]"}`}
                      >
                        {m.label}
                      </div>
                      <div
                        className={`text-xs mt-0.5 ${m.highlight ? "text-white/60" : "text-[var(--color-muted-foreground)]"}`}
                      >
                        {m.sub}
                      </div>
                    </div>
                ...
                return (
                  <>
                    ...
                    <div className="card-base p-6 border-2 border-[var(--color-primary)]/20 bg-gradient-to-r from-orange-50 to-white">
                      ...
                      <p className="text-5xl font-display font-bold text-[var(--color-primary)] mb-1">
                        {a.introductionRequestRate}%
                      </p>
                      <p className="text-sm text-[var(--color-muted-foreground)]">
                      ...
                      <div className="flex items-end gap-1 h-32">
                        {a.scoreDistribution.map(bucket => {
                          const maxCount = Math.max(
                            ...a.scoreDistribution.map(b => b.count),
                            1
                          );
                          ...
                          return (
                            ...
                            >
                              ...
                              />
                              <span className="text-[10px] text-[var(--color-muted-foreground)] rotate-0">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="card-base p-6">
                        ...
                        <div className="space-y-2">
                          {Object.entries(a.countryBreakdown)
                            .sort(([, a], [, b]) => b - a)
                            .slice(0, 8)
                            .map(([country, count]) => (
                              <div
                                ...
                                <div className="flex-1">
                                  ...
                                  <div className="h-1.5 bg-[var(--color-muted)] rounded-full">
                                    <div
                                      ...
                                      style={{
                                        width: `${Math.round((count / a.total) * 100)}%`,
                                      }}
                                    />
                                  </div>
                                </div>
                      ...
                      <div className="card-base p-6">
                        <h3 className="font-display font-bold text-[var(--color-navy)] mb-4 flex items-center gap-2">
                          <PieChart className="w-5 h-5 text-blue-500" /> Funding
                          Method
                        </h3>
                        <div className="space-y-2">
                          {Object.entries(a.fundingBreakdown)
                            .sort(([, a], [, b]) => b - a)
                            .map(([method, count]) => (
                              <div
                                key={method}
                      ...
                      <div className="card-base p-6">
                        <h3 className="font-display font-bold text-[var(--color-navy)] mb-4 flex items-center gap-2">
                          <Target className="w-5 h-5 text-green-500" /> Pilot
                          Goal
                        </h3>
                        <div className="space-y-2">
                          {Object.entries(a.goalBreakdown)
                            .sort(([, a], [, b]) => b - a)
                            .map(([goal, count]) => (
                              <div
                                key={goal}
                      ...
                      <div className="card-base p-6">
                        <h3 className="font-display font-bold text-[var(--color-navy)] mb-4 flex items-center gap-2">
                          <PieChart className="w-5 h-5 text-amber-500" /> Budget
                          Range
                        </h3>
                        <div className="space-y-2">
                          {Object.entries(a.budgetBreakdown)
                            .sort(([, a], [, b]) => b - a)
                            .map(([budget, count]) => (
                              <div
                                key={budget}
                    ...
                    {Object.keys(a.sourceBreakdown ?? {}).length > 0 && (
                      <div className="card-base p-6">
                        ...
                        <div className="space-y-2">
                          {Object.entries(a.sourceBreakdown ?? {})
                            .sort(
                                ...
                                <div className="flex-1">
                                  ...
                                  <div className="h-1.5 bg-[var(--color-muted)] rounded-full">
                                    <div
                                      ...
                                      style={{
                                        width: `${Math.round(((count as number) / a.total) * 100)}%`,
                                      }}
                                    />
                                  </div>
                                </div>
                    ...
                    {a.leadsPerDay.length > 0 && (
                      <div className="card-base p-6">
                        <h3 className="font-display font-bold text-[var(--color-navy)] mb-4">
                          Assessments per Day (Last 30 Days)
                        </h3>
                        <div className="flex items-end gap-1 h-24">
                          {a.leadsPerDay.map(d => {
                            const maxCount = Math.max(
                              ...a.leadsPerDay.map(x => x.count),
                              1
                            );
                            const height = Math.round(
                            ...
                            return (
                              ...
                              >
                                <div
                                  className="w-full rounded-t-sm bg-[var(--color-navy)]/60"
                        ...
                        <p className="text-xs text-[var(--color-muted-foreground)] mt-2">
                          {a.leadsPerDay[0]?.date} →{" "}
                          {a.leadsPerDay[a.leadsPerDay.length - 1]?.date}
                        </p>
                      </div>
                    )}
                  </>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
              {[
                ...
                },
                {
                  ...
                  value:
                    total > 0
                      ? `${Math.round((leads.filter(l => l.status === "Converted").length / total) * 100)}%`
                      : "0%",
                          ...
                          return (
                            ...
                            >
                              <span className="inline-flex items-center gap-1">
                                {h === "Intent" ? (
                                  <span title="Intent Score — commercial readiness signal (admin only)">
                                    ⚡ Intent
                                  </span>
                                ) : (
                                  h
                    ...
                    <tbody>
                      {leads.map(lead => (
                        ...
                        >
                          ...
                          <td className="px-4 py-3 text-[var(--color-muted-foreground)] max-w-32 truncate">
                            {lead.pilotGoal ?? "—"}
                          </td>
                          ...
                          <td className="px-4 py-3">
                            <span
                              className={`font-bold ${sortBy === "leadScore" ? "text-[var(--color-primary)]" : "text-[var(--color-navy)]"}`}
                            >
                              {lead.leadScore}
                            </span>
                            <span className="text-xs text-[var(--color-muted-foreground)]">
                          ...
                          <td className="px-4 py-3">
                            {(lead.intentScore ?? 0) > 0 ? (
                              <span
                                className={`font-bold ${sortBy === "intentScore" ? "text-amber-600" : "text-amber-500"}`}
                              >
                                {lead.intentScore}
                              </span>
                            ) : (
