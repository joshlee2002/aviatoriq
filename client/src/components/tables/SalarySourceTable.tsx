...
import { PILOT_SALARY_SOURCES_2026, type PilotSalarySource } from "../../data/pilotSalarySources2026";
...
const payBasisBadge = (basis: PilotSalarySource["payBasis"]) => {
  ...
  return (
    <span className={`inline-block rounded-full px-2 py-0.5 text-xs font-medium ${map[basis] ?? "bg-gray-100 text-gray-600"}`}>
      {basis}
    </span>
  );
};
...
export const SalarySourceTable: React.FC<SalarySourceTableProps> = ({
  ...
  const rows = country
    ? PILOT_SALARY_SOURCES_2026.filter((r) => r.country.toLowerCase().includes(country.toLowerCase()))
  ...
  return (
    <div className={`space-y-4 ${className}`}>
          ...
          <tbody className="divide-y divide-gray-100 bg-white">
            {rows.map((row, i) => (
              <tr key={i} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-4 py-3 font-semibold text-gray-900 text-xs">{row.salaryRange}</td>
      ...
      <div className="rounded-md border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800">
        <p className="font-semibold mb-1">Salary figures are ranges, not guarantees</p>
        <p>
          Actual pay depends on airline, seniority, fleet type, roster, and allowances. "Annual (basic)" figures
          exclude duty pay, sector pay, and allowances. "Annual (total remuneration)" includes these but is an
          estimate. Upper-end figures typically reflect senior captains — not representative of a new entrant.
          Always verify current pay scales directly with the airline or via the relevant pilots' union.
        </p>
      </div>

      <p className="text-xs text-gray-400">
        Data last checked: June 2026. Sources: BALPA, ALPA, AFAP, and individual airline careers pages.
      </p>
    </div>
  );
};
