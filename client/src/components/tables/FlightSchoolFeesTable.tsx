  ...
  const rows =
    trainingType === "all"
      ...
      : UK_FLIGHT_SCHOOL_FEES_2026.filter((r) =>
          r.trainingType.toLowerCase().includes(trainingType.toLowerCase())
        );
          ...
          <tbody className="divide-y divide-gray-100 bg-white">
            {rows.map((row, i) => (
              <tr key={i} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-4 py-3 text-gray-700">{row.trainingType}</td>
                ...
                <td className="px-4 py-3 text-xs text-gray-600">{row.feeIncludes}</td>
              </tr>
            ))}
          </tbody>
        </table>
