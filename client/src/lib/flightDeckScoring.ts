...
function getArchetype(input: FlightDeckInput): {
  archetype: string;
  archetypeDescription: string;
} {
  ...
  if (
    commitment === "applied" ||
    commitment === "visited_school" ||
    commitment === "trial_lesson"
  ) {
    return {
  ...
  if (
    commitment === "researched_schools" &&
    barrier === "information_overload"
  ) {
    return {
  ...
  if (
    commitment === "just_dreaming" ||
    commitment === "watched_videos" ||
    barrier === "confidence"
  ) {
    return {
...
function getBarrierAdvice(key: BarrierKey): string {
  const map: Record<BarrierKey, string> = {
    ...
    confidence:
      "Confidence is the most underestimated barrier in aviation. Almost every pilot who qualified felt exactly the way you do before they started. The first step — a trial lesson — costs less than £200 and changes most people's perspective entirely.",
    medical:
      "Medical concerns stop more people than they should. Most common health conditions don't automatically disqualify you from a Class 1 medical. An Aviation Medical Examiner assessment takes less than a day and gives you a definitive answer. Uncertainty is worse than knowing.",
    ...
    career_risk:
      "The risk calculation is real, and it deserves a serious answer. The key question is: what is the cost of not doing it? Many career changers find that the modular route lets them keep their income while training — reducing the financial risk significantly.",
    information_overload:
      "More information isn't the answer. A structured assessment that maps your specific situation to a specific path is. The goal isn't to know everything about pilot training — it's to know what the right next step is for you.",
    unknown:
      "The most useful thing you can do right now is understand your specific situation clearly. A full career assessment takes 5 minutes and gives you a personalised roadmap built around your actual profile.",
  };
  return map[key];
}

function getStrongestAsset(input: FlightDeckInput): {
  strongestAsset: string;
  assetDescription: string;
} {
  ...
  if (
    ...
    return {
      ...
      assetDescription:
        "You've already taken action — which puts you ahead of the majority of people who only ever think about it. That's not a small thing.",
    };
  }
  if (budget === "funded_or_sponsored" || budget === "serious_savings") {
    return {
      ...
      assetDescription:
        "You're in a stronger financial position than most aspiring pilots. That removes the biggest practical barrier for the majority of people in your situation.",
    };
  }
  if (medical === "have_class1" || medical === "no_concerns") {
    return {
      ...
      assetDescription:
        "Medical uncertainty stops a lot of people before they start. You've already cleared that hurdle — or have no reason to worry about it.",
    };
  }
  if (
    ...
    return {
      ...
      assetDescription:
        "You know exactly what you want. That clarity is more valuable than it sounds — most people spend years in indecision before committing to a direction.",
    };
  }
  if (risk === "ready_to_commit" || risk === "no_commitments") {
    return {
      ...
      assetDescription:
        "You're not held back by career obligations or financial dependants. That gives you more options — and more speed — than most people at your stage.",
    };
  }
  return {
    strongestAsset: "Awareness",
    assetDescription:
      "You're asking the right questions at the right time. Most people who become pilots started exactly where you are — curious, uncertain, and looking for a clear path forward.",
  };
}
...
function getHeadline(
  ...
  if (score >= 50) {
    if (barrierKey === "cost")
      return "The gap between you and the cockpit is mostly financial.";
    if (barrierKey === "confidence")
      return "The gap between you and the cockpit is mostly in your head.";
    if (barrierKey === "medical")
      return "One medical assessment could change everything.";
    if (barrierKey === "career_risk")
      return "You have a real path — it just needs a plan that protects your income.";
    return "You have a real path — it just needs a plan.";
  }
  if (score >= 35) {
    if (archetype === "The Dreamer")
      return "You've been thinking about this long enough. It's time to find out if it's real.";
...
function getSubheadline(
  ...
  if (score >= 72)
    return `Your profile matches someone who could realistically become a ${label}. Your biggest challenge right now is taking the next concrete step.`;
  if (score >= 55)
    return `Becoming a ${label} is achievable from where you are. Your main obstacle is ${getBarrierLabel(barrierKey).toLowerCase()} — which is a solvable problem, not a permanent one.`;
  if (score >= 40)
    return `The path to becoming a ${label} is real from your position. But it will require a clear plan that addresses your specific situation — not generic advice.`;
